...
class ResNetDecoder(nn.Module):
    def __init__(
        ...
        # Decoder layers (same as your original)
        self.groups = groups
        ...
        # ---------------------------
        # ADD SELF-ATTENTION on final decoder output (d3 equivalent)
        # ---------------------------
        self.use_attention = use_attention

        if use_attention:
            # Decoder final feature (f1) channels = last layer's output planes * expansion
            d3_channels = layer_planes[0] * block.expansion   # 64 * 4 = 256 for ResNet50 decoder
            self.attn_d3 = SelfAttention2d(
                d3_channels,
    ...
    # ======================================================================
    #                    ResNet-style layer builder (unchanged)
    # ======================================================================
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        # Decoder uses Upsample for stride > 1
    ...
    # ======================================================================
    def forward(self, x):
        # f3 → f2 → f1 (your original decoder)
        f3 = self.layer3(x)
        f2 = self.layer2(f3)
        f1 = self.layer1(f2)   # deepest decoder feature → d3

        # Apply self-attention to decoder d3 (f1)
        if self.use_attention:
            try:
                f1 = self.attn_d3(f1)
            except Exception:
                pass  # safe fallback if shapes mismatch

        return f1, f2, f3  # (d3, d2, d1)
        ...
        for k in state_dict:
            if k in model_state:
                if state_dict[k].shape != model_state[k].shape:
                    print(f"Skip loading parameter {k}: expected {model_state[k].shape}, got {state_dict[k].shape}")
                else:
...
def resnet50_decoder(pretrained=False, progress=True, inplanes=[2048], **kwargs):
    return _resnet(
        "resnet50",
        Bottleneck,
        [3, 4, 6, 3],
